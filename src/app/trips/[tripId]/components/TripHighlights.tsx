...
const TripHighlights = ({highlights}: TripHighlightsProps) => {
  return (
    <div className="flex flex-col p-5 lg:p-0 lg:mt-12">
      ...
      <div className="flex flex-wrap gap-y-3 lg:mt-3">
        {highlights.map((highlights, index) => (
          <div key={highlights} className="flex items-center gap-2 lg:gap-3 w-1/2">
            <Image src="/check-icon.png" width={15} height={15} alt={highlights} />

            <p className="text-grayPrimary text-xs lg:text-base">{highlights}</p>
          </div>
        ))}
